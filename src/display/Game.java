...
public class Game extends JPanel implements KeyListener {
	...
	private static int speed = 50, spmSize = 60, waveHeight = 50;
	private static int base = 400, xStart = 1000;
	private long point = 0, lastPress = 0;

	private Superman spm = new Superman(100, base - 50);
	...
	public void paint(Graphics g) {
		try {
			...
			drawSupermanHealth(g2);
			g2.drawImage(spm.getImage(), spm.x, spm.y, spmSize, spmSize, null);
			// ----Wave----
	...
	private void drawSupermanHealth(Graphics2D g2) {
		try {
			...
			g2.drawLine(60, 30, 60 + spm.health, 30);
			g2.setColor(Color.white);
	...
	private void drawWave(Wave wave, Graphics2D g2) {
		...
		if (Event.checkHit(spm, wave, spmSize, waveHeight)) {
			g2.setColor(new Color(241, 98, 69));
			g2.fillRect(0, 0, 1000, 1000);
			spm.health -= 20;
			if (spm.health <= 0) {
				display.endGame(this.point);
				spm.health = new Superman().health;
				this.point = 0;
			}
		}
	}
